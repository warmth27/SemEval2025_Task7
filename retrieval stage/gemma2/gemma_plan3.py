...
model = SentenceTransformer("/opt/data/private/bge-multilingual-gemma2", model_kwargs={"torch_dtype": torch.float16}).to(device)

# 加载post和fact_check JSON文件
print("Loading post and fact_check data...")
with open("/opt/data/private/SemEval2025_Task7/json_dataset/fact_check/monolingual_fact_checks_spa.json", "r", encoding="utf-8") as f:
    fact_check_data = json.load(f)
with open("/opt/data/private/SemEval2025_Task7/json_dataset/post_dev/monolingual_posts_dev_spa.json", "r", encoding="utf-8") as f:
...
# 将嵌入转换为PyTorch张量
query_embeddings_tensor = torch.tensor(query_embeddings, dtype=torch.float16, device=device)
document_embeddings_tensor = torch.tensor(document_embeddings, dtype=torch.float16, device=device)
...
# 保存结果为JSON文件
output_file = "gemma_task1_plan3_spa.json"
