...
# 批量生成嵌入的函数
def generate_embeddings_in_batches(input_texts, tokenizer, max_length, model, batch_size):
    embeddings_list = []
    for i in range(0, len(input_texts), batch_size):
        ...
        # 提取嵌入并进行归一化
        embeddings = last_token_pool(batch_dict.last_hidden_state, batch_dict['attention_mask'])
...
# 初始化分词器和模型（指定torch_dtype为半精度）
tokenizer = AutoTokenizer.from_pretrained('/opt/data/private/e5-mistral-7b-instruct')
model = AutoModel.from_pretrained('/opt/data/private/e5-mistral-7b-instruct', torch_dtype=torch.float16).to(device)
...
embeddings = generate_embeddings_in_batches(input_texts, tokenizer, model, batch_size=batch_size)
...
with open("e5_mistral_results_task3_plan3_tha.json", "w", encoding="utf-8") as f:
