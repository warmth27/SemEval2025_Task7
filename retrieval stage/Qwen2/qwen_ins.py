...
with open("/opt/data/private/SemEval2025_Task7/json_dataset/fact_check/monolingual_fact_checks_eng.json", "r", encoding="utf-8") as f:
    fact_check_data = json.load(f)
with open("/opt/data/private/SemEval2025_Task7/json_dataset/post_dev/monolingual_posts_dev_eng.json", "r", encoding="utf-8") as f:
    posts_data = json.load(f)

# 任务描述
instruction = 'Given a social media post, retrieve relevant fact-checks for the given post.' #task1
# instruction = 'Given a social media post below, retrieve the 10 most relevant fact-checks for the given post.' # task2
# instruction = 'Given a social media post, retrieve relevant fact-checks' # task3
# instruction = '' # task4
# instruction = 'Given a web search query, retrieve the 10 most relevant passages from a collection. Each passage should answer the query or provide information closely related to it. Relevance is determined by the semantic similarity between the query and the passage\'s content. Return the results ranked by relevance.' #task5
# instruction = 'Given a social media post, Return the fact-checks ranked by relevance, ensuring that the top 10 results are the most contextually appropriate for the given post.' #task6
# instruction = 'Please retrieve fact-checks from the document collection that are relevant to the following query, and return the most relevant content to the query in descending order of relevance.' #task7
# Function to format the task description and query
...
# 保存结果为JSON文件
output_file = "qwen_task1_plan3_eng.json"
